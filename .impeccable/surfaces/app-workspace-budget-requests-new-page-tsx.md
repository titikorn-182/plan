...
THESIS: การสร้างคำของบเป็นสมุดงานต้นทางหน้าเดียว ไม่ใช่ wizard; ผู้ประสานงานต้องเห็นความครบถ้วน นำเข้าข้อมูล ตรวจแถว แจกแจงค่าใช้จ่ายหลายรายการ และบันทึกหรือส่งได้ในกระแสต่อเนื่องเดียว
...
EDIT: `/budget-requests/[id]/edit` ใช้ workbook form ชุดเดียวกับหน้าสร้าง โดยแสดงหัวข้อแก้ไขและรหัสคำขอ พร้อมเติมข้อมูลทั้ง 6 หมวดจากรายการเดิม; คง id, version, ปีงบประมาณ, budget cycle และ organization scope สำหรับบันทึกกลับคำขอเดิม ไม่สร้างซ้ำ; ซ่อนการนำเข้าไฟล์ในหน้าแก้ไขเพื่อป้องกันการแทนที่ข้อมูลโดยไม่ตั้งใจ และยังคงกฎสิทธิ์และสถานะที่แก้ไขได้เดิม

EDIT PRESERVATION: โหลดรายการค่าใช้จ่าย ผู้ร่วมรับผิดชอบ SDGs และคำอธิบายครบ พร้อมคงข้อมูลเก่าที่ไม่มีช่องแสดง รวมถึง metadata แหล่งเงินของแต่ละรายการ; dropdown แสดงค่าที่บันทึกไว้แต่ไม่มีในตัวเลือกปัจจุบันโดยกำกับ “ข้อมูลเดิม” ให้ตรวจแก้ได้ ไม่ล้างเงียบ ๆ; คำขอเก่าที่ไม่มี expenseItems แสดงยอดรวมและสรุปหมวดเดิม และจะคงข้อมูลนี้ไว้จนกว่าผู้ใช้เลือก “เปลี่ยนเป็นรายการค่าใช้จ่าย” และบันทึกเอง โดยไม่เดาหมวดย่อย
...
PROJECT TEAM: ใช้หัวหน้าโครงการหนึ่งคนเป็นข้อมูลบังคับพร้อมตำแหน่ง และมีรายการผู้ร่วมรับผิดชอบแบบเพิ่มหรือลบได้สูงสุด 50 คน โดยแต่ละรายการมีชื่อ-นามสกุลและตำแหน่ง; ช่องว่างเริ่มต้นไม่ถูกบันทึก ส่วนรายการที่เริ่มกรอกแล้วต้องมีชื่อก่อนส่ง; ข้อมูลเก็บใน proposalDetails.projectMembers ขณะที่คอลัมน์ผู้เห็นชอบและผู้อนุมัติเดิมยังอ่านจากไฟล์นำเข้าเพื่อความเข้ากันได้แต่ไม่แสดงในฟอร์มใหม่

OUTCOMES ORDER: ส่วนเป้าหมาย เหตุผล และระยะเวลาเรียงเป็น หลักการและเหตุผล → วัตถุประสงค์ → ประโยชน์ที่คาดว่าจะได้รับ → กลุ่มเป้าหมาย → วันที่เริ่มและวันที่สิ้นสุด เพื่อให้ผู้กรอกเล่าที่มา เป้าหมาย ผลลัพธ์ ผู้รับประโยชน์ และเวลาอย่างต่อเนื่อง
...
PLAN SELECTS: รหัสและชื่อของผลผลิต แผนปฏิบัติการ และโครงการ/กิจกรรมใช้ Dropdown จาก master data ของปีงบประมาณที่เลือก; เลือกรหัสหรือชื่อแล้วจับคู่อีกช่องอัตโนมัติ เลือกกิจกรรมแล้วเติมแผนระดับบน และกรองรายการย่อยตามแผนที่เลือก; เมื่อเปลี่ยนระดับบนให้ล้างเฉพาะระดับย่อยที่ไม่สัมพันธ์กัน; server ตรวจคู่รหัส–ชื่อและลำดับชั้นทั้งบันทึกฉบับร่างและส่งคำขอ
...
READINESS: rail ด้านขวาแสดง 7 checks พร้อมจำนวนที่พร้อม ได้แก่ organization+fiscal year, ความสอดคล้องของ organization scope กับรหัสหน่วยงานย่อย, ชื่อ+ประเภทโครงการ, ความสอดคล้องของรหัสและชื่อโครงสร้างแผน, หัวหน้าโครงการ, วงเงิน, และหลักการเหตุผล; readiness เป็นคำแนะนำก่อนส่ง ไม่ใช่การแทน server validation
...
FEEDBACK: สถานะอ่านไฟล์ ผลการนำแถวไปใช้ และผลบันทึก/ส่งอยู่ใน polite live regions; file/row errors และ warnings แสดง inline ใกล้ preview; field errors จาก server เชื่อมด้วย `aria-invalid` และ `aria-describedby` พร้อมนำ focus ไปช่องผิดช่องแรก; ฟอร์มคงค่าที่กรอกและไม่ให้ native reset ล้าง Dropdown เมื่อ action ส่งผลกลับ; progress 0–100 มี accessible label และค่าปัจจุบัน
