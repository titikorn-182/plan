  ...
  return (
    <form action={action}>
      ...
      <button
        ...
        disabled={pending}
      >
        {pending ? <LoaderCircle className="animate-spin" size={13} /> : <Check size={13} />}
        อ่านแล้ว
      </button>
    </form>
  );
  ...
  return (
    <div className="space-y-5">
      <section className="grid border border-stone-200 bg-white sm:grid-cols-[1fr_auto]">
        ...
        <form
          action={action}
          className="flex items-center border-t border-stone-200 p-4 sm:border-t-0 sm:border-l"
        >
      ...
      {state.message ? (
        <p
          className={`border px-4 py-3 text-xs ${state.success ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-red-200 bg-red-50 text-red-800"}`}
        >
          {state.message}
        </p>
      ) : null}
      <RegisterSection
        ...
        aside={<span className="text-xs text-stone-500">ล่าสุด {notifications.length} รายการ</span>}
      >
        <ul className="divide-y divide-stone-200">
          {notifications.map((item) => (
            <li
              className={`grid gap-3 p-4 sm:grid-cols-[32px_1fr_auto] sm:items-center ${item.read ? "bg-white" : "bg-[#fff7f0]"}`}
              key={item.id}
            >
              <span
                className={`grid size-8 place-items-center rounded-full ${item.read ? "bg-stone-100 text-stone-500" : "bg-orange-100 text-[#c9440b]"}`}
              >
                <Bell size={15} />
              </span>
              <span>
                <b className="block text-sm">{item.title}</b>
                <span className="mt-1 block text-xs leading-5 text-stone-600">{item.body}</span>
                <small className="mt-1 block text-[10px] text-stone-500">{item.createdAt}</small>
              </span>
              {!item.read ? (
