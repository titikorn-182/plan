...
function ReadButton({ id }: { id: string }) {
  const [, action, pending] = useActionState(markNotificationAction, {} as OperationState);
...
export function NotificationsView({ notifications }: { notifications: NotificationRow[] }) {
  const [state, action, pending] = useActionState(markNotificationAction, {} as OperationState);
