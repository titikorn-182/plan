      ...
      >
        <div className="grid gap-2 border-b border-stone-200 bg-[#fffdfa] p-3 md:grid-cols-[minmax(260px,1fr)_190px_auto_auto]">
          <label className="flex h-10 items-center gap-2 border border-stone-300 bg-white px-3 focus-within:border-[#dc4f12]">
            ...
            <input className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-stone-400" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ค้นหารหัส รายการ หรือหน่วยงาน" />
          </label>
          <label className="relative flex h-10 items-center border border-stone-300 bg-white px-3">
            ...
            <select className="w-full appearance-none bg-transparent pr-6 text-sm outline-none" value={status} onChange={(event) => setStatus(event.target.value)}>
          ...
          <button className="inline-flex h-10 items-center justify-center gap-2 border border-stone-300 bg-white px-4 text-sm font-semibold hover:border-[#dc4f12] hover:text-[#c9440b]" type="button"><Download size={16} /> ส่งออก</button>
          ...
          <table className="w-full min-w-[980px] border-collapse text-left text-xs">
            <thead className="bg-stone-50 text-stone-600">
              <tr className="border-b border-stone-200">
                <th className="w-12 px-4 py-3"><input type="checkbox" aria-label="เลือกทุกรายการ" /></th>
            ...
            <tbody>
              {rows.map((item) => (
                <tr className="border-b border-stone-200 bg-white transition-colors hover:bg-orange-50/60" key={item.id}>
                  <td className="px-4 py-3"><input type="checkbox" aria-label={`เลือก ${item.title}`} /></td>
                  ...
                  <td className="px-3 py-3"><button className="grid size-8 place-items-center border border-transparent hover:border-stone-300 hover:text-[#c9440b]" type="button" aria-label={`เปิด ${item.id}`}><ArrowUpRight size={16} /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <footer className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 text-xs text-stone-500">
          <span>หน้า 1 จาก {Math.max(1, Math.ceil(rows.length / 10))}</span>
          <div className="flex gap-1"><button className="border border-stone-300 px-3 py-1.5 disabled:opacity-40" type="button" disabled>ก่อนหน้า</button><button className="bg-[#cf430c] px-3 py-1.5 text-white" type="button">1</button><button className="border border-stone-300 px-3 py-1.5 disabled:opacity-40" type="button" disabled={rows.length <= 10}>ถัดไป</button></div>
        </footer>
      </RegisterSection>
    </div>
  );
