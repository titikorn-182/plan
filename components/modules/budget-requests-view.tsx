...
export function BudgetRequestsView({ requests }: { requests: BudgetRequest[] }) {
  ...
  function exportCsv() {
    ...
    downloadCsv(csv, `budget-requests-${new Date().toISOString().slice(0, 10)}.csv`);
  }
  ...
  const summary = [
    ...
    [
      "รอตรวจสอบ",
      requests.filter((item) => item.status === "รอตรวจสอบ").length,
      "อยู่ระหว่างตรวจความครบถ้วน",
    ],
    [
      "รออนุมัติ",
      requests.filter((item) => item.status === "รออนุมัติ").length,
      "อยู่ในคิวผู้มีอำนาจอนุมัติ",
    ],
    [
      "ส่งกลับแก้ไข",
      requests.filter((item) => item.status === "ส่งกลับแก้ไข").length,
      "รอเจ้าของคำขอดำเนินการ",
    ],
  ] as const;
    ...
    <div className="space-y-5">
      <section className="grid border border-stone-200 bg-white sm:grid-cols-2 xl:grid-cols-4">
        {summary.map(([label, value, note], index) => (
          <article
            className={`flex min-h-24 items-center gap-4 px-5 py-4 ${index < 3 ? "border-b border-stone-200 sm:border-r xl:border-b-0" : ""}`}
            key={label}
          >
            <strong className="text-3xl font-bold text-[#d8460b] tabular-nums">
              {formatThaiInteger(value)}
            </strong>
            <span>
              <b className="block text-sm">{label}</b>
              <small className="mt-1 block text-[11px] text-stone-500">{note}</small>
            </span>
          </article>
        ))}
      </section>
      ...
      <RegisterSection
        ...
        aside={
          <span className="text-xs text-stone-500">
            แสดง {rows.length} จาก {requests.length} รายการ
          </span>
        }
      >
        <div className="grid gap-2 border-b border-stone-200 bg-[#fffdfa] p-3 md:grid-cols-[minmax(260px,1fr)_190px_auto_auto]">
          ...
          <button
            className="inline-flex h-10 items-center justify-center gap-2 border border-stone-300 bg-white px-4 text-sm font-semibold hover:border-[#dc4f12] hover:text-[#c9440b]"
            type="button"
            onClick={exportCsv}
          ...
          <Link
            className="inline-flex h-10 items-center justify-center gap-2 bg-[#cf430c] px-4 text-sm font-semibold text-white hover:bg-[#ad3507]"
            href="/budget-requests/new"
          >
            <FilePlus2 size={16} /> สร้างคำขอ
          </Link>
        </div>
